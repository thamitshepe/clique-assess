  ...
  const items = useMemo(() => {
    return fights.map((fight, index) => {
      const matchedPrediction = predictions && predictions.find((prediction) => {
        const homeFighterName = fight.homeFighter?.name || '';
        const awayFighterName = fight.awayFighter?.name || '';
  
        const homeFighterMatch = prediction['Home Fighter']?.toLowerCase().includes(homeFighterName.toLowerCase());
        const awayFighterMatch = prediction['Away Fighter']?.toLowerCase().includes(awayFighterName.toLowerCase());
